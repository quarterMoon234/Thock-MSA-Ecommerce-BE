...
### [2026-03-11 00:00] Ingress 관리자 IP 화이트리스트에 운영 관리자 IP 추가
- 변경 유형: `보안정책`
- 대상 환경: `kubernetes`
- 대상 컴포넌트: ingress middleware (Traefik `ipAllowList`)
- 작업자: ops-admin
- 목적/배경:
  - 운영 중 다른 관리자의 대시보드 접속을 허용하기 위해 관리자 허용 IP 범위 확장 필요
- 변경 내용:
  - `deploy-kubernetes/ingress/ip-whitelist-middleware.yaml`의 `sourceRange`에 추가 관리자 IP 반영
  - 기존 관리자 IP 허용 규칙은 유지한 상태로 화이트리스트 항목만 증설
- 검증 방법:
  - 허용 대상 IP에서 대시보드 경로 접근 시 `200/302` 응답 확인
  - 비허용 IP에서 동일 경로 접근 시 `403` 응답 확인
- 결과:
  - `성공`
  - 상세: 추가된 관리자 IP에서 운영 대시보드 접근 가능, 기존 차단 정책 정상 유지
- 영향도:
  - 사용자 영향: `없음` (관리자 경로 접근 정책만 변경)
  - 장애 등급: `해당없음`
- 롤백 여부:
  - `불필요`
- 후속 TODO:
  - 관리자 IP 변경 요청 프로세스와 반영 이력을 운영 문서에 연계 관리
- 참조:
  - PR:
  - Commit/Image Tag:
  - 이슈: 운영 관리자 접근 허용 범위 조정
...
| v1.0.4 | 2026-03-11 | ops-admin | ingress middleware 관리자 IP 화이트리스트 확장 이력 추가 |
